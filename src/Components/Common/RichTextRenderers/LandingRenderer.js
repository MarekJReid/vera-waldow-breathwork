    ...
    renderNode: {
      [INLINES.HYPERLINK]: (node, children) => (
        <a
          href={node.data.uri}
          target="_blank"
          style={{ borderBottom: `.75px solid red` }}
        >
          {children}
        </a>
      ),
      [BLOCKS.HEADING_1]: (node, children) => (
        <Heading1 marginBottom="sm">{children}</Heading1>
      ),
      ...
      [BLOCKS.PARAGRAPH]: (node, children) => <BC1 color="one">{children}</BC1>,
    },
  };
